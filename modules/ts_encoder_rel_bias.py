...
class TS_encoder_layer(nn.Module):
    ...
    def forward(self, src:Tensor, prev:Optional[Tensor]=None, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None,actual_N=None,actual_ch=None) -> Tensor:
        # pre-norm
        if self.pre_norm:
        ...
        if self.res_attention:
            src2, attn, scores = self.self_attn(src, src, src, prev, mask=attn_mask,actual_N=actual_N,actual_ch=actual_ch)
        else:
            src2, attn = self.self_attn(src, src, src, mask=attn_mask,actual_N=actual_N,actual_ch=actual_ch)
...
class TST_encoder(nn.Module):
    def __init__(self,d_model=None, n_heads=2,d_ff=256,norm='BatchNorm', attn_dropout=0., dropout=0., activation='gelu', n_layers=2, res_attention=False, pre_norm=False,store_attn=False):
    ...
    def forward(self, src: torch.Tensor,actual_N=None,actual_ch=None):
        output=src ### initial input tensor
        scores=None
        
        if self.res_attention:
            for mod in self.layers:output,scores = mod(output, prev=scores,actual_N=actual_N,actual_ch=actual_ch)
            return output
        
        else:
            for mod in self.layers: output = mod(output,actual_N=actual_N,actual_ch=actual_ch)
            return output

##input_shape --> B,C*N,p
class PatchTSTEncoder(nn.Module):
    def __init__(self,patch_len=516,n_layers=1,d_model=128,n_heads=8,shared_embedding=False,d_ff=256,
        ...
        self.W_P=nn.Linear(patch_len,self.d_model) ### channel mixing
    ...
    def forward(self, x:torch.Tensor):
        ...
        z= self.encoder(u,actual_N=max_N,actual_ch=max_ch) ### call the ts_encoder passing the actual_seq_len 
...
"""
ts_encoder = llm_projection(conv_ts_encoder,64,trans_ts_encoder,512,1024,3072)
###summary(ts_encoder,input_data=(test_x))

ts_embeddings = ts_encoder(test_x)
